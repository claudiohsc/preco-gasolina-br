...
# armazenando anos e estados para o dropdown;
estados = []
anos = []
...
df_exp_imp = pd.read_excel("importacoes-exportacoes-petroleo-2000-2022_21.xlsx")
...
def tabela_anos():
  ...
  #para cada ano realiza a media do ano
...
def update_graph_prod(estado, ano):
  
  tabela_estado = tabela_por_estado(df_producao, estado, int(ano)) #recebe os dois valores: producao e mês
  
  fig = px.bar(tabela_estado, x='MÊS', y='PRODUÇÃO', title=f'Produção de Petróleo em {estado}.', template="plotly_dark")
